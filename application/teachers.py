...
def create_class():
    ...
    # request is post
    insert_db("insert into classes (teacher_id, name) values (?, ?);", [
              session['id'], request.form['name']])
    classData = query_db(
        "select id, name from classes order by id desc limit 1", one=True)
    flash("Your class, %s, was created with an id of %s." %
          (classData[1], classData[0]))
...
@validate_teacher
def create_topic():
    insert_db("insert into topics (name, class_id) values (?, ?);",
              [request.form['name'], request.form['class']])
...
def topic_page(topic_id=None):
    topicData = query_db("select name from topics where id=?", [
                         topic_id], one=True)
...
def create_assignment():
    ...
    insert_db("insert into assignments (name, description, due_date, topic_id) values (?, ?, ?, ?);", [
              request.form['name'], request.form['description'], request.form['due_date'], request.form['topic']])
...
def assignment_page(assignment_id):
    assignmentInfo = query_db(
        "SELECT name, due_date, description FROM assignments WHERE id=?;", [assignment_id])
...
@validate_teacher
def create_quiz():
    insert_db("insert into quizzes (topic_id, creator_id, name) values(?, ?, ?);", [
              request.form['topic'],  session['id'], request.form['name']])
...
def quiz_page(quiz_id=None):
    questionData = query_db(
        "select question_text, correct_answer, a_answer_text, b_answer_text, c_answer_text, d_answer_text from questions where quiz_id=?;", [quiz_id])
    questions = []
...
def create_question(quiz_id=None):
    insert_db("INSERT INTO questions (correct_answer, question_text, a_answer_text, b_answer_text, c_answer_text, d_answer_text, quiz_id) VALUES (?, ?, ?, ?, ?, ?, ?);", [str(
        request.form['answer']), str(request.form['question']), str(request.form['a_answer']), str(request.form['b_answer']), str(request.form['c_answer']), str(request.form['d_answer']), str(quiz_id)])
...
def create_grade(class_id):
    insert_db("INSERT INTO assignment_grades (student_id, assignment_id, grade) VALUES (?, ?, ?);", [
