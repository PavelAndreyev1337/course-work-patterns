...
def getStudentClasses():
    classData = query_db(
        "SELECT classes.id, classes.name FROM classes JOIN roster ON roster.class_id = classes.id where people_id=?;", [session['id']])
    classes = []
...
def getStudentGrades(class_id):
    ...
    """
    grades = []
    quizGrades = query_db("SELECT quizzes.name, grade FROM quiz_grades JOIN quizzes ON quiz_grades.quiz_id=quizzes.id JOIN topics on quizzes.topic_id=topics.id JOIN classes ON topics.class_id=classes.id WHERE student_id=? AND topics.class_id=?;", [
                          session['id'], class_id])
    ...
    assignmentGrades = query_db(
        "SELECT assignments.name, grade FROM assignment_grades JOIN assignments ON assignment_grades.assignment_id=assignments.id JOIN topics on assignments.topic_id=topics.id JOIN classes ON topics.class_id=classes.id WHERE student_id=? AND topics.class_id=?;", [session['id'], class_id])
...
def getTeacherClasses():
    ...
    """
    classData = query_db(
        "select id, name from classes where teacher_id = ?;", [session['id']])
    classes = []
...
def getAllTeacherTopics():
    ...
    """
    topicData = query_db(
        "select topics.id, topics.name, classes.name from topics join classes on topics.class_id=classes.id where teacher_id=?;", [session['id']])
    topics = []
...
def getClassTopics(class_id):
    ...
    """
    topicData = query_db(
        "select id, name from topics where class_id=?", [class_id])
    topics = []
    for topic in topicData:
        holderDict = {}
        holderDict['id'] = topic[0]
...
def getTeacherAssignments():
    ...
    """
    assignmentData = query_db(
        "select assignments.id, assignments.name, assignments.due_date from assignments join topics on assignments.topic_id=topics.id join classes on topics.class_id=classes.id where teacher_id=?;", [session['id']])
    assignments = []
...
def getClassAssignments(class_id):
    ...
    """
    assignmentData = query_db(
        "select id, name, due_date from assignments where topic_id=(select id from topics where class_id=?);", [class_id])
    assignments = []
    for assignment in assignmentData:
        holderDict = {}
        holderDict['id'] = assignment[0]
...
def getTeacherQuizzes():
    ...
    """
    quizData = query_db(
        "select id, name from quizzes where creator_id=?;", [session['id']])
    quizzes = []
...
def getTopicQuizzes(topic_id):
    ...
    """
    quizData = query_db(
        "select id, name from quizzes where topic_id=?;", [topic_id])
    quizzes = []
    for quiz in quizData:
        holderDict = {}
        holderDict['id'] = quiz[0]
...
def getClassQuizzes(class_id):
    ...
    """
    quizData = query_db(
        "SELECT quizzes.id, quizzes.name FROM quizzes JOIN topics ON topics.id=quizzes.topic_id WHERE topics.class_id=?;", [class_id])
    quizzes = []
    for quiz in quizData:
        holderDict = {}
        holderDict['id'] = quiz[0]
...
def getRegisteredStudents(class_id):
    ...
    """
    studentData = query_db(
        "SELECT people.id, name FROM people JOIN roster ON roster.people_id=people.id WHERE roster.class_id=?;", [class_id])
    people = []
...
def getClassGrades(class_id):
    ...
    grades = []
    quizGrades = query_db(
        "SELECT people.name, quizzes.name, grade FROM quiz_grades JOIN people ON quiz_grades.student_id=people.id JOIN quizzes ON quiz_grades.quiz_id=quizzes.id JOIN topics ON quizzes.topic_id=topics.id JOIN classes ON topics.class_id=classes.id WHERE classes.id=?;", [class_id])
    for grade in quizGrades:
        holderDict = {}
        ...
        grades.append(holderDict)
    assignmentGrades = query_db(
        "SELECT people.name, assignments.name, grade FROM assignment_grades JOIN people ON assignment_grades.student_id=people.id JOIN assignments ON assignment_grades.assignment_id=assignments.id JOIN topics ON assignments.topic_id=topics.id JOIN classes ON topics.class_id=classes.id WHERE classes.id=?;", [class_id])
    for grade in assignmentGrades:
        holderDict = {}
        holderDict['student_name'] = grade[0]
