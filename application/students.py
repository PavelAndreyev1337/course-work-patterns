...
def student_class_join():
    insert_db("INSERT INTO roster (people_id, class_id) VALUES (?, ?);", [
...
def student_class_page(class_id):
    className = query_db("SELECT name FROM classes WHERE id=?;", [
                         class_id], one=True)
...
def student_topic_page(topic_id):
    topicName = query_db("SELECT name FROM topics WHERE id=?;", [
                         topic_id], one=True)
...
def student_quiz_page(quiz_id):
    ...
    result = query_db("SELECT grade from quiz_grades WHERE quiz_id=? AND student_id=?;", [
                      quiz_id, session['id']], one=True)  # check if the person has already taken the test
    if result == None:
        questionData = query_db(
            "SELECT id, question_text, a_answer_text, b_answer_text, c_answer_text, d_answer_text FROM questions WHERE quiz_id=?;", [quiz_id])
        questions = []
        for question in questionData:
            holderDict = {}
            holderDict['id'] = question[0]
            holderDict['text'] = question[1]
            holderDict['answers'] = [str(question[2]), str(
                question[3]), str(question[4]), str(question[5])]
...
def grade_quiz(quiz_id):
    ...
    for i in range(len(answers)):
        correctAnswer = query_db("SELECT correct_answer FROM questions WHERE id=?;", [
                                 question_ids[i]], one=True)
        if int(correctAnswer[0]) == answers[i]:
            correct += 1.0
        questions += 1.0
    percent = (correct/questions)*100
    insert_db("INSERT INTO quiz_grades (student_id, quiz_id, grade) VALUES (?, ?, ?);", [
              session['id'], quiz_id, percent])
...
def student_assignment_page(assignment_id):
    assignmentData = query_db("SELECT name, description, due_date FROM assignments WHERE id=?;", [
